...
_global_ void vecAdd(int n, int* a, int* b, int* c) {
    int i = threadIdx.x;
...
int main() {
    ...
    int threads = 1024;  // Quantidade de threads por bloco (máximo até 1024).

    int blocks = 1;  // Quantidade de blocos.
    ...
    */
    vecAdd << <blocks, threads> >> (n, d_a, d_b, d_c);
